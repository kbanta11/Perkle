...
class _SlidingPanelState extends State<SlidingPanel> with SingleTickerProviderStateMixin{
  ...
  void initState(){
    ...
    _sc?.addListener((){
      if((widget.isDraggable ?? true) && !_scrollingEnabled)
        _sc?.jumpTo(0);
    });
    ...
    return Stack(
      ...
      children: <Widget>[
        ...
        widget.body != null ? AnimatedBuilder(
          animation: _ac ?? AnimationController(vsync: this),
          builder: (context, child){
            return Positioned(
              top: (widget.parallaxEnabled ?? false) ? _getParallax() : 0.0,
        ...
        !(widget.backdropEnabled ?? false) ? Container() : GestureDetector(
          onVerticalDragEnd: (widget.backdropTapClosesPanel ?? false) ? (DragEndDetails dets){
            // only trigger a close if the drag is towards panel close position
            if((-1) * dets.velocity.pixelsPerSecond.dy > 0)
              _close();
          } : null,
          onTap: (widget.backdropTapClosesPanel ?? false) ? () => _close() : null,
        ...
        !_isPanelVisible ? Container() : _gestureHandler(
          child: AnimatedBuilder(
            ...
            builder: (context, child) {
              return Container(
                height: _ac!.value * ((widget.maxHeight ?? 50) - (widget.minHeight ?? 10)) + (widget.minHeight ?? 10),
                margin: widget.margin,
                padding: widget.padding,
                decoration: (widget.renderPanelSheet ?? false) ? BoxDecoration(
                ...
                Positioned(
                  ...
                  child: Container(
                    ...
                    child: widget.collapsed == null ? Container() : FadeTransition(
                      opacity: Tween(begin: 1.0, end: 0.0).animate(_ac ?? AnimationController(vsync: this)),
  ...
  double _getParallax(){
    return (_ac?.value ?? 0) * ((widget.maxHeight ?? 0) - (widget.minHeight ?? 0)) * (widget.parallaxOffset ?? 0);
  }
  ...
  Widget _gestureHandler({Widget? child}){
    if (!(widget.isDraggable ?? true)) return (child ?? Container());
  ...
  void _onGestureSlide(double dy){
    ...
    if(!_scrollingEnabled){
      _ac!.value += dy / ((widget.maxHeight ?? 10) - (widget.minHeight ?? 50));
    }

    // if the panel is open and the user hasn't scrolled, we need to determine
    // whether to enable scrolling if the user swipes up, or disable closing and
    // begin to close the panel if the user swipes down
    if(_isPanelOpen && (_sc?.hasClients ?? false) && (_sc?.offset ?? 0) <= 0){
      setState(() {
        if(dy < 0){
  ...
  void _onGestureEnd(Velocity v){
    ...
    double visualVelocity = -v.pixelsPerSecond.dy / ((widget.maxHeight ?? 10) - (widget.minHeight ?? 50));
    ...
    if(v.pixelsPerSecond.dy.abs() >= minFlingVelocity){
      ...
      // snapPoint exists
      if((widget.panelSnapping ?? false) && widget.snapPoint != null){
        if(v.pixelsPerSecond.dy.abs() >= kSnap*minFlingVelocity || minDistance == d2Snap)
          _ac!.fling(velocity: visualVelocity);
        else
          _flingPanelToPosition(widget.snapPoint ?? 0, visualVelocity);

        // no snap point exists
      }else if(widget.panelSnapping ?? false){
        _ac!.fling(velocity: visualVelocity);
  ...
  Future<void> _animatePanelToSnapPoint({Duration? duration, Curve curve = Curves.linear}){
    ...
    return _ac!.animateTo(widget.snapPoint ?? 0, duration: duration, curve: curve);
  }
...
class PanelController{
  _SlidingPanelState _panelState = _SlidingPanelState();
