...
class _ProfilePageMobileState extends State<ProfilePageMobile> {
  ...
  build(BuildContext context) {
    ...
    return widget.userId == null ? Center(child: CircularProgressIndicator(),) : MultiProvider(
      providers: [
        StreamProvider<PerklUser?>(create: (_) => UserManagement().streamUserDoc(widget.userId), initialData: null),
      ],
...
class _UserInfoSectionState extends State<UserInfoSection> {
  ...
  Widget build(BuildContext context) {
    ...
    Widget followUnfollowButton(User? currentUser, PerklUser? pageUser)  {
      ...
      if(pageUser?.followers != null && (pageUser?.followers?.contains(currentUser?.uid) ?? false)) {
        return ButtonTheme(
            height: 25,
            minWidth: 50,
            child: OutlinedButton(
                                ...
                                Column(
                                  children: <Widget>[
                                    InkWell(
                                      ...
                                      onTap: () {
                                        if(widget.user?.followers != null && (widget.user?.followers?.length ?? 0)> 0) {
                                ...
                                Column(
                                  children: <Widget>[
                                    InkWell(
                                        ...
                                        onTap: () {
                                          if(widget.user?.following != null && (widget.user?.following?.length ?? 0) > 0) {
                                            Navigator.push(context, MaterialPageRoute(
                                              builder: (context) =>
