...
class PlayerAudioHandler extends BaseAudioHandler
  ...
  Future<void> _init() async {
    ...
    if(currentItem != null) {
      MediaItem currentMediaItem = Helper().getMediaItemFromJson(currentItem);
    ...
    if(speed != null) {
      playbackState.add(playbackState.valueWrapper?.value.copyWith(speed: speed) ?? PlaybackState());
    }

    List<dynamic>? localQueue = await _localService.getData('queue');
    if(localQueue != null && localQueue.length > 0) {
      queue.add(localQueue.map((item) => Helper().getMediaItemFromJson(item)).toList());
    }
  ...
  Future<void> updateTimeListened(Duration? time, MediaItem? item) async {
    ...
    if(item.extras?['clipId'] != null) {
      //get item by clipId
      updateItem = listeningHistory.firstWhereOrNull((_) => _.extras?['clipId'] == item.extras?['clipId']);
      if(updateItem != null) {
      ...
      //get item by id
      updateItem = listeningHistory.firstWhereOrNull((_) => _.id == item.id);
    ...
    await _historyLocalService.setData('items', listeningHistory.map((e) => Helper().mediaItemToJson(e)).toList());
    /*
  ...
  updateLocalQueue(List<MediaItem>? queue) async {
    List<Map>? queueList = queue?.map((MediaItem item) {
      ...
      return Helper().mediaItemToJson(item);
    }).toList();
  ...
  _playCurrentItem({MediaItem? item}) async {
    ...
    List<MediaItem> listeningHistory = await _historyLocalService.getData('items').then((dynamic itemList) {
      ...
      return (itemList as List).map((item) => Helper().getMediaItemFromJson(item)).toList();
    });
    ...
    await _localService.setData('current_item', Helper().mediaItemToJson(item));
    ...
    await player.setSpeed(playbackState.valueWrapper?.value.speed ?? 1.0);
      ...
      if(d.inSeconds > currentPosition.inSeconds) {
        ...
        playbackState.add(playbackState.valueWrapper?.value.copyWith(
            controls: [MediaControl.rewind,
              ...
              queue != null && !(await queue.isEmpty) ? MediaControl.skipToNext : null].whereNotNull().toList(),//.where((element) => element != null).toList() ?? [],
            playing: true,
    ...
    player.processingStateStream.listen((ProcessingState processState) async {
      if(processState == ProcessingState.completed) {
        ...
        if(queue != null && queue.valueWrapper?.value != null && (queue.valueWrapper?.value?.length ?? 0) > 0) {
          ...
          playbackState.add(playbackState.valueWrapper?.value.copyWith(
              ...
              playing: false,
            updatePosition: item?.duration ?? Duration(),
          ) ?? PlaybackState());
        }
      }
    });
  ...
  Future<void> fastForward([Duration? interval]) async {
    ...
    if (position.inSeconds + 30 >= (duration?.inSeconds ?? 1)) {
      await player.seek(Duration(seconds: (duration?.inSeconds ?? 15) - 15));
      player.play();
    } else {
  ...
  Future<void> play() async {
     ...
     */
    await player.setSpeed(playbackState.valueWrapper?.value.speed ?? 1);
 ...
 */
  ...
  Future<void> setSpeed(double speed) async {
    bool isPlaying = playbackState.valueWrapper?.value.playing ?? false;
    if(isPlaying) {
